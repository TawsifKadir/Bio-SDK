...
public class FingerprintImage {
	...
	public FingerprintImage decode(byte[] image, int width , int height) {
		...
		for (int i = 0; i < image.length; i++) {
			Bits[i * 4] = Bits[i * 4 + 1] = Bits[i * 4 + 2] = image[i]; // Invert the source bits
	...
	public FingerprintImage decode(byte[] image) {
		...
		matrix = new DoubleMatrix(bmpW, bmpH);
