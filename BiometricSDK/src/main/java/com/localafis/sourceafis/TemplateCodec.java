...
import com.localafis.fingerprintio.TemplateFormat;
import com.localafis.fingerprintio.ansi378.Ansi378Fingerprint;
import com.localafis.fingerprintio.ansi378.Ansi378Minutia;
import com.localafis.fingerprintio.ansi378.Ansi378MinutiaType;
import com.localafis.fingerprintio.ansi378.Ansi378Template;
import com.localafis.fingerprintio.ansi378.Ansi378v2009Am1Fingerprint;
import com.localafis.fingerprintio.ansi378.Ansi378v2009Am1Template;
import com.localafis.fingerprintio.ansi378.Ansi378v2009Fingerprint;
import com.localafis.fingerprintio.ansi378.Ansi378v2009Minutia;
import com.localafis.fingerprintio.ansi378.Ansi378v2009MinutiaType;
import com.localafis.fingerprintio.ansi378.Ansi378v2009Template;
import com.localafis.fingerprintio.iso19794p2.Iso19794p2Fingerprint;
import com.localafis.fingerprintio.iso19794p2.Iso19794p2Minutia;
import com.localafis.fingerprintio.iso19794p2.Iso19794p2MinutiaType;
import com.localafis.fingerprintio.iso19794p2.Iso19794p2Template;
...
abstract class TemplateCodec {
	...
	static final Map<com.localafis.fingerprintio.TemplateFormat, TemplateCodec> ALL = new HashMap<>();
	static {
		ALL.put(com.localafis.fingerprintio.TemplateFormat.ANSI_378, new Ansi378Codec());
		ALL.put(com.localafis.fingerprintio.TemplateFormat.ANSI_378_2009, new Ansi378v2009Codec());
		ALL.put(com.localafis.fingerprintio.TemplateFormat.ANSI_378_2009_AM1, new Ansi378v2009Am1Codec());
	...
	private static class Ansi378Codec extends TemplateCodec {
		@Override byte[] encode(List<MutableTemplate> templates) {
			...
			// src: iotemplate.width = templates.stream().mapToInt(t -> t.size.x).max().orElse(500);
			iotemplate.width = StreamSupport.stream(templates).mapToInt(t -> t.size.x).max().orElse(500);
			// src: iotemplate.height = templates.stream().mapToInt(t -> t.size.y).max().orElse(500);
			iotemplate.height = StreamSupport.stream(templates).mapToInt(t -> t.size.y).max().orElse(500);
			...
			iotemplate.fingerprints = IntStream.range(0, templates.size())
				...
				.collect(toList());
			 */
			iotemplate.fingerprints = IntStreams.range(0, templates.size())
					.mapToObj(n -> encode(n, templates.get(n)))
					.collect(toList());
		...
		@Override List<MutableTemplate> decode(byte[] serialized, boolean permissive) {
			...
			return iotemplate.fingerprints.stream()
				...
				.collect(toList());
			 */
			return StreamSupport.stream(iotemplate.fingerprints)
					.map(fp -> decode(fp, iotemplate, resolution))
					.collect(toList());
		}
		static Ansi378Fingerprint encode(int offset, MutableTemplate template) {
			...
			iofingerprint.minutiae = template.minutiae.stream()
				...
				.collect(toList());
			 */
			iofingerprint.minutiae = StreamSupport.stream(template.minutiae)
				.map(m -> encode(m))
				.collect(toList());
		...
		static MutableTemplate decode(Ansi378Fingerprint iofingerprint, Ansi378Template iotemplate, Resolution resolution) {
			...
			template.minutiae = iofingerprint.minutiae.stream()
				...
				.collect(toList());
			 */
			template.minutiae = StreamSupport.stream(iofingerprint.minutiae)
				.map(m -> decode(m, resolution))
				.collect(toList());
			return template;
		}
	...
	private static class Ansi378v2009Codec extends Ansi378Codec {
		...
		@Override List<MutableTemplate> decode(byte[] serialized, boolean permissive) {
			...
			return new Ansi378v2009Template(serialized, permissive).fingerprints.stream()
				...
				.collect(toList());
		     */
			return StreamSupport.stream(new Ansi378v2009Template(serialized, permissive).fingerprints)
					.map(fp -> decode(fp))
					.collect(toList());
		}
	...
	private static class Ansi378v2009Am1Codec extends Ansi378v2009Codec {
		...
		@Override List<MutableTemplate> decode(byte[] serialized, boolean permissive) {
			...
			return new Ansi378v2009Am1Template(serialized, permissive).fingerprints.stream()
				...
				.collect(toList());
		     */
			return StreamSupport.stream(new Ansi378v2009Am1Template(serialized, permissive).fingerprints)
					.map(fp -> decode(fp))
					.collect(toList());
		}
