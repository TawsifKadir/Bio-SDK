...
public abstract class FingerprintTransparency implements AutoCloseable {
	...
	static FingerprintTransparency current() {
		// src: return Optional.ofNullable(current.get()).orElse(NOOP);
		FingerprintTransparency ft = current.get();
		return ft == null ? NOOP : ft;
	}
	...
	@SuppressWarnings("unused") private static class CborSkeleton {
		...
		CborSkeleton(Skeleton skeleton) {
			...
			// src: this.minutiae = skeleton.minutiae.stream().map(m -> m.position).collect(toList());
			this.minutiae = StreamSupport.stream(skeleton.minutiae).map(m -> m.position).collect(toList());
			/* src:
	...
	void logEdgeHash(Int2ObjectMap<List<IndexedEdge>> hash) {
		log("edge-hash", () -> {
			...
			return Arrays.stream(hash.keySet().toIntArray())
				...
				.collect(toList());
		     */
			return J8Arrays.stream(hash.keySet().toIntArray())
					.sorted()
					.mapToObj(key -> {
						CborHashEntry entry = new CborHashEntry();
						entry.key = key;
	...
	@SuppressWarnings("unused") private static class CborPair {
		...
		static List<CborPair> roots(int count, MinutiaPair[] roots) {
			// src: return Arrays.stream(roots).limit(count).map(p -> new CborPair(p.probe, p.candidate)).collect(toList());
			return J8Arrays.stream(roots).limit(count).map(p -> new CborPair(p.probe, p.candidate)).collect(toList());
		}
	}
	/*
	...
	@SuppressWarnings("unused") private static class CborPairing {
		...
		CborPairing(int count, MinutiaPair[] pairs, List<MinutiaPair> support) {
			...
			// src: tree = Arrays.stream(pairs).limit(count).skip(1).map(CborEdge::new).collect(toList());
			tree = J8Arrays.stream(pairs).limit(count).skip(1).map(CborEdge::new).collect(toList());
			// src: this.support = support.stream().map(CborEdge::new).collect(toList());
			this.support = StreamSupport.stream(support).map(CborEdge::new).collect(toList());
		}
	}
