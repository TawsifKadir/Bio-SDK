...
import com.kit.photocapture.model.ModelManager;
...
public class PhotoCaptureActivity2 extends CameraActivity implements CameraBridgeViewBase.CvCameraViewListener2 {
    ...
    public Mat onCameraFrame(CameraBridgeViewBase.CvCameraViewFrame inputFrame) {
        ...
        // Fix front camera orientation (mirror + vertical flip)
        if (mIsFrontCamera) {
            Core.flip(mRgba, mRgba, 1); // Horizontal flip (mirror)
            Core.flip(mRgba, mRgba, 0); // Vertical flip
        }
    ...
    private void initializeFaceDetector() {
        try {
            FaceDetectionModel faceDetectionModel = ModelManager.getFaceDetectionModel(this);
            mFaceDetector = faceDetectionModel.getDetector();
